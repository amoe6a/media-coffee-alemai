...
check("upsert auto-registered category", any(c["name"] == "Drinks" for c in db.list_categories()))
check("add category", db.add_category("Напитки")[0] is True)
check("category Cyrillic case-insensitive dup", db.add_category("напитки") == (False, "exists"))
check("add subcategory", db.add_subcategory("Напитки", "Кофе")[0] is True)
check("subcategory Cyrillic dup", db.add_subcategory("Напитки", "кофе") == (False, "exists"))
nid, nmsg = db.create_menu_item("Напитки", "Кофе", "Капучино", 1200.0)
check("manual item created at stock 0", nmsg == "created" and db.get_item(nid)["quantity"] == 0)
check("manual item dup rejected", db.create_menu_item("Напитки", "Кофе", "капучино", 1.0)[0] is None)
check("rename item", db.rename_item(nid, "Капучино большой")[0] is True)
db.set_price(nid, 1500.0)
check("set price", db.get_item(nid)["price"] == 1500.0)
check("customer_categories hides empty", {c["name"] for c in db.customer_categories()} >= {"Drinks", "Напитки"})
check("customer_subcategories filters", [s["name"] for s in db.customer_subcategories("Напитки")] == ["Кофе"])
# cascade delete a category removes its items + subcategories
cap_cat_id = db.get_category_id_by_name("Напитки")
_, n_items, n_subs = db.category_delete_counts(cap_cat_id)
check("cascade counts", n_items == 1 and n_subs == 1)
db.delete_category(cap_cat_id)
check("category gone after delete", db.get_category_id_by_name("Напитки") is None)
check("cascade removed item", db.get_item(nid) is None)
